...
func (c *Client) GetOrCreateColumnID(owner, repoName string, proj Project) (int64, error) {
	...
	log.Printf("Column %q not found in project %q. Creating it...\n", proj.ColumnName, proj.ProjectName)
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
...
func (c *Client) SyncPRProjects(
	...
	for _, branchCfgs := range moveToProjectsForLabelsXORed {
		...
		if len(lblsSet) > 1 {
			log.Printf("Multiple labels set not applying any action for projects. Pick one of: %s\n", lblsSet)
			continue
		}
